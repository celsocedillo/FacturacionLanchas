...
exports.findLastFacturaId = function() {
    return conn.db.one(`SELECT DISTINCT MAX(id_factura) as last
                        FROM public.factura;`);
};

exports.findFacturaById = function(factura_id) {
    return conn.db.one(`SELECT id_factura, status, cant_gastos, subtotal, total_impuestos, total, a.cliente_id, b.nombre, b.telefono,
                        b.direccion, b.email
                        FROM public.factura a 
                        LEFT JOIN cliente b on b.cliente_id = a.cliente_id
                        WHERE id_factura = $1;`, [factura_id]);
};

exports.addFactura = function(factura_id, cant_gastos, subtotal, total_impuestos, total, cliente_id) {
  console.log("los gastos ",factura_id, cant_gastos, subtotal, total_impuestos, total, cliente_id)  
  conn.db.none(`INSERT INTO public.factura(
      id_factura, status, cant_gastos, subtotal, total_impuestos, total, cliente_id)
      VALUES ($1, true, $2, $3, $4, $5, $6);`, [factura_id, cant_gastos, subtotal, total_impuestos, total, cliente_id]);
};
exports.actualizarEstadoFactura = function(factura_id, status) {
  console.log("los gastos ",factura_id)  
  conn.db.none(`UPDATE public.factura
                SET status= $2
                WHERE id_factura = $1;`, [factura_id, status]);
};




exports.DBfindHousingWayDetByHw = function(housing_way_id) {
    return conn.db.any(`SELECT housingway_detail_id, housing_way_id, TO_CHAR(scheduled_date, 'DD/MM/YYYY') as scheduled_date, 
                        ...
                        WHERE housing_way_id = $1 order by scheduled_date DESC`, [housing_way_id]);
};
